...
namespace Demo
{
    public class Wallet
    {
        ...
        private static (Amount paid, Amount remainToPay, List<Money> remainingMoneys) Withdraw(
            (Amount paid, Amount remainToPay, List<Money> remainingMoneys) state, Money money)
        {
            Amount paid = money.Of(state.remainToPay.Currency).Withdraw(state.remainToPay).paid;
            return (
                state.paid.Add(paid),
                state.remainToPay.Subtract(paid),
                new List<Money>(state.remainingMoneys)
