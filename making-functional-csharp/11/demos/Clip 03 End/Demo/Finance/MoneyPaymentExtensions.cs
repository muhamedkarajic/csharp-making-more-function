...
namespace Demo.Finance
{
    ...
    {
        public static (Amount paid, Option<Money> remaining) Withdraw(this Option<Money> money, Amount toPay) =>
            money is Some<Money> some
                ? ((Money)some).Withdraw(toPay).Map(tuple => (tuple.paid, (Option<Money>)tuple.remaining))
                : (Amount.Zero(toPay.Currency), None.Value);

        public static (Amount paid, Money remaining) Withdraw(this Money money, Amount expense)
        {
            switch (money)
            {
